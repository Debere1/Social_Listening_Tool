...
#sidebar settings
st.sidebar.header('Sociolite `Dashboards`' )
...
if rad == 'Twitter':
    ...
    #col4.metric(label=" ### Potential Reach", value=int(reach), delta="")
    ...
    with c1:
        ...
        plot_data = st.multiselect('Select data', ['Likes', 'Comments','Engagements'])
        
        grp= df[['Date','Impressions', 'Likes', 'Engagements', 'Comments','Potential Reach']].groupby(['Date']).sum().reset_index()
        
        st.write(px.line(df, x= 'Date', y= plot_data ,color_discrete_sequence=px.colors.qualitative.Plotly_r))
...
if rad == 'Facebook':
    ...
    with c1:
        st.markdown('### Line graph')
        plo_data = st.multiselect('Select data', ['Likes on posts', 'Comments on posts',"Post reach (page likers [fans] only)","Post impressions (page likers [fans] only)","Shares on posts"])
        ...
        st.write(px.line(graph, x= 'Post creation date', y= plo_data,color_discrete_sequence=px.colors.qualitative.Antique))
...
st.sidebar.markdown('''
---
Created with Gumption❤️ by [ExploreAI].
''') 